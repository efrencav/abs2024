...
(function ($) {
  ...
  function ServicesSection($el = $(document)) {
    if (dsnGrid.isMobile()) return;
    ($el.hasClass("Services-with-img") ? $el.find('.dsn-Services') : $el.find(".Services-with-img")).each(function () {
      const ServicesItem = $(this).find(".Services-item"),
        onEnter = function () {
          ServicesItem.not(this).removeClass('active');
          this.classList.add("active");
          ServicesItem.not(this).find('.Services_description ').slideUp(400);
          $(this).find('.Services_description ').slideDown(400);
        };

      ServicesItem.first().addClass("active");
      ServicesItem.on('mouseenter', onEnter);
      dsnGrid.killAjax(function () {
        ServicesItem.off('mouseenter', onEnter);
      });
    });
  }
  /**
